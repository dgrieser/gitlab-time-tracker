  ...
  class IssueSelectorDialog extends ModalDialog.ModalDialog {
    ...
    _loadProjects() {
      ...
      try {
        ...
        log(
          `GitLab Issue Selector: Using token length: ${token ? token.length : 0}`,
        );
        ...
        this._httpSession.send_and_read_async(
          ...
          (session, result) => {
            try {
              ...
              log(
                `GitLab Issue Selector: API response status code: ${message.status_code}`,
              );
              log(
                `GitLab Issue Selector: API response length: ${response.length}`,
              );
              ...
              if (message.status_code === 200) {
                ...
                log(
                  `GitLab Issue Selector: Loaded ${this._projects.length} projects`,
                );
                this._updateProjectList();
                this._hideLoading();
              } else {
                log(
                  `GitLab Issue Selector: Error fetching projects: ${message.status_code}`,
                );
              ...
              log(
                `GitLab Issue Selector: Error parsing projects: ${e.message}`,
              );
    ...
    _selectProject(project, widget) {
      ...
      widget.style =
        "padding: 8px; border-radius: 3px; background-color: #4a90d9;";
    ...
    _onSelect() {
      if (!this._selectedProject || !this._selectedIssue) {
        Main.notify(
          this._("GitLab Time Tracking"),
          this._("Please select a project and an issue"),
        );
        return;
      }
